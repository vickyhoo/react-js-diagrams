  ...
  _createClass(DiagramWidget, [{
      ...
      if (action instanceof _actions.SelectingAction && multiselectDrag) {
        ...
        action.selectionModels.forEach(function (model) {
          if (model.model instanceof _Common.NodeModel || model.model instanceof _Common.PointModel) {
            var _diagramModel$getZoom3 = diagramModel.getZoomLevel(),
                _diagramModel$getZoom4 = _slicedToArray(_diagramModel$getZoom3, 2),
                zoomX = _diagramModel$getZoom4[0],
                zoomY = _diagramModel$getZoom4[1];

            model.model.x = model.initialX + (event.pageX - _this3.state.action.mouseX) / (zoomX / 100);
            model.model.y = model.initialY + (event.pageY - _this3.state.action.mouseY) / (zoomY / 100);
          }
        });
        ...
        var _diagramModel$getZoom5 = diagramModel.getZoomLevel(),
            _diagramModel$getZoom6 = _slicedToArray(_diagramModel$getZoom5, 2),
            zoomX = _diagramModel$getZoom6[0],
            zoomY = _diagramModel$getZoom6[1];

        diagramModel.setOffset(action.initialOffsetX + (event.pageX - left - this.state.action.mouseX) / (zoomX / 100), action.initialOffsetY + (event.pageY - top - this.state.action.mouseY) / (zoomY / 100));
    ...
    value: function render() {
      var _props6 = this.props,
          diagramEngine = _props6.diagramEngine,
          style = _props6.style;
